...
export default function Block({ id, image, title, description, skip, like, unLike }) {
    return (
        <div className="block">
            {
                JSON.parse(localStorage.getItem('Favourites')) ? JSON.parse(localStorage.getItem('Favourites')).map(product => product.id === id ?
                    <svg viewBox="0 0 91 91" onClick={() => unLike(id)}>
                        <g>
            ...
            <div className="row">
                {
                    skip ? <Button txt='Skip' handlerClick={skip} /> : ''
                }
                {
                    like ? <Button txt='Like' handlerClick={() => like(id, image, title, description)} /> : ''
                }
                {
                    unLike ? <Button txt='Unlike' handlerClick={() => unLike(id)}/> : ''
                }
            </div>
        </div>
    )
